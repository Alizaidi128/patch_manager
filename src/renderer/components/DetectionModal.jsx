...
function PatchCard({ patchResult, allResults, onDeployed }) {
  const [deployState, setDeployState] = useState(null) // null | 'checking' | 'blocked' | 'deploying' | 'done' | 'error'
  ...
  const isDeployed         = patchResult.detectedStatus === 'deployed'
...
export default function DetectionModal({ appId, appName, onClose }) {
  ...
  function runDetect() {
    setState('loading')
    setData(null)
    window.api.invoke('patch:detect-all', { appId })
      .then(d  => { setData(d);            setState('done') })
      .catch(e => { setErrorMsg(e.message); setState('error') })
  }

  useEffect(() => { runDetect() }, [appId])

  const visibleResults = !data ? [] : data.results.filter(p => {
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && state !== 'loading' && onClose()}>
      <div className="modal modal-lg detection-modal">
        <div className="modal-header">
          <h3>Deployment Status Check — {appName}</h3>
          <button className="modal-close" onClick={onClose}>✕</button>
        </div>

        <div className="modal-body det-modal-body">
          {state === 'loading' && (
            <div className="det-loading">
              <div className="fetch-spinner" />
              <span>Comparing patch files with app directory…</span>
            </div>
          )}
        ...
        <div className="modal-footer">
          <button className="btn btn-secondary" onClick={runDetect} disabled={state === 'loading'}>
            Re-detect
          </button>
